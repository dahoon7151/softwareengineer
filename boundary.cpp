...
//=====================// ApplyRecruit Controll //====================
void UI_ApplyRecruit::startInterface(){
...
void UI_ApplyRecruitInfo::startInterface(){
    fprintf(out_fp, "3.2. 등록된 채용 정보 조회\n");
}

void Control_ApplyRecruitInfo::call_startInterface(MemberList* memberlist, RecruitInfoList* recruitlist, string* curID, string* curPW){
    ui_applyrecruitinfo = new UI_ApplyRecruitInfo(this, memberlist, recruitlist, curID, curPW);
}

void Control_ApplyRecruitInfo::showRecruitInfo(MemberList* memberlist, RecruitInfoList* recruitlist, string* curID, string* curPW){
    int numRecruitInfo = recruitlist->getNumRecruitInfo();
    for (int i = 0; i < numRecruitInfo; i++) {
        RecruitInfo* recruitinfo = recruitlist->recruitlist[i];

        cout << "Task: " << recruitinfo ->getTask() << endl;
        cout << "Apply Number: " << recruitinfo->getApplyNumber() << endl;
        cout << "Deadline: " << recruitinfo->getDeadline() << endl;
    }
}
